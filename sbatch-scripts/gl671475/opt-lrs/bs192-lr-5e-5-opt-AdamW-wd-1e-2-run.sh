...
#SBATCH --output=%j-bs192-lr-5e-5-optim-AdamW-wd-1e-2-output.log
#SBATCH --error=%j-bs192-lr-5e-5-optim-AdamW-wd-1e-2-error.log
#SBATCH --nodes=1
...
python3 main.py \
    ...
    --base_lr 0.00005 \
    --name "bs_192_lr_5e-5_opt_AdamW_wd_1e-2_ep_20" \
    --wandb "run" \
    --job_id ${SLURM_JOB_ID} \
...
mv "./sbatch-scripts/${USERNAME}/opt-lrs/${SLURM_JOB_ID}-"*".log" ${REGAT_SAVE_EXPERIMENT_TRAIN_PATH}
